...
def list_to_txt_list( usr_list: List[str], delimiter: str = '*'):
    result = ''
    for line in usr_list:
        result = f'{result}\n{delimiter} {line}' if result != '' else f'{delimiter} {line}'
    return result

def color_result(result, output_type: str = 'Text'):
    if output_type == 'str':
        if result == 'success':
...
def tree_report(inventory_anta: InventoryDevices, testcases: List[str]) -> None:
    tree = Tree("Test Result by Host")
    for device in inventory_anta.get_inventory(format_out='list'):
        results = manager.get_result_by_host(str(device.host), output_format='list')
        test_tree = tree.add(str(device.host))
...
def report(inventory_anta: InventoryDevices, testcases: List[str]) -> None:
    ...
    for result in manager.get_results(output_format='list'):
        ...
        message = list_to_txt_list(result.messages) if len(result.messages) > 0 else 'N/A'
        table.add_row(result.test,
                      str(result.host),
                      state,
                      message
                      )
    ...
    for device in inventory_anta.get_inventory(format_out='list'):
        results = manager.get_result_by_host(
            str(device.host), output_format='list')
        nb_failure = len(
            [result for result in results if result.result == 'failure'])
        list_failure = [
...
def cli_manager():
    ...
    parser.add_argument('--search_test', '-st', required=False,
    ...
    parser.add_argument('--full', '-f', required=False, action='store_true',
                        help='Display all test cases results')

    parser.add_argument('--devices', '-d', required=False, action='store_true',
    ...
    parser.add_argument('--old_report', required=False, action='store_true',
                        help='Use builtin report (not maintained anymore)')

    parser.add_argument('--tree', required=False, action='store_true',
...
if __name__ == '__main__':
    ...
    if cli_options.old_report:
        report(inventory_anta=inventory_anta, testcases=list_tests_name)

    if cli_options.table:
        reporter = ReportTable()
        if cli_options.full:
            console.print(reporter.report_all(result_manager=manager,
                          host=cli_options.search, testcase=cli_options.search_test))
        if cli_options.testcases:
            console.print(reporter.report_summary_tests(result_manager=manager, testcase=cli_options.search_test))
        if cli_options.devices:
            console.print(reporter.report_summary_hosts(result_manager=manager, host=cli_options.search))
    ...
    if cli_options.tree:
        tree_report(inventory_anta=inventory_anta, testcases=list_tests_name)
