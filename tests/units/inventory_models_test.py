...
from anta.inventory.models import AntaInventoryHost, AntaInventoryNetwork, AntaInventoryRange, AntaInventoryInput
from tests.data.utils import generate_test_ids_dict
from tests.data.json_data import INVENTORY_MODEL_HOST, INVENTORY_MODEL_NETWORK, INVENTORY_MODEL_RANGE, INVENTORY_MODEL
...
class Test_AntaInventoryInputModel():
    ...
    #     f1.parent.mkdir()
    #     f1.touch()
    #     f1.write_text(yaml.dump(content, allow_unicode=True))
    #     return str(f1)
    ...
    def test_anta_inventory_intput_valid(self, inventory_def):
        ...
        try:
            ...
            if 'networks' in inventory_def['input'].keys():
                logging.info('Loading %s into AntaInventoryInput networks section', str(inventory_def['input']['networks']))
