...
def credentialz_fact(state: FeatureState) -> AvailableFact[GnsiCredentialzFact]:
    ...
    return GnsiCredentialzFact(state).available(SOURCE)
...
def test_sa165_assessment_contract() -> None:
    ...
    assert isinstance(_assess_sa165(missing_version, credentialz_fact(FeatureState.DISABLED)), NotAffectedResult)
    ...
    assert isinstance(_assess_sa165(missing_version, credentialz_fact(FeatureState.ENABLED)), ErrorResult)
