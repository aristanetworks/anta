...
- The [refresh()](../api/device.md#anta.device.AsyncEOSDevice.refresh) coroutine tries to open a TCP connection on the eAPI port and update the `is_online` attribute accordingly. If the TCP connection succeeds, it sends a `show version` command to gather the hardware model and EOS version of the device and updates the `established`, `hw_model`, `platform`, and `version` attributes. For known modular chassis, refresh also collects `show module` once and augments `platform` with normalized supervisor, switch-card, and line-card identities. Fixed systems do not receive this additional command. If module evidence is unavailable, the device remains established and the structured identity records which component roles are incomplete.
