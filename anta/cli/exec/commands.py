...
@click.option(
    "--configure/--not-configure", help="Ensure device has 'aaa authorization exec default local' configured (required for SCP)", default=False, show_default=True, required=False
)
@click.option("--tags", "-t", default=DEFAULT_TAG, help="List of tags using coma as separator: tag1,tag2,tag3", type=str, required=False)
# Debug stuf
@click.option(
    "--log-level", "--log", help="Logging level of the command", default="info", show_default=True, type=click.Choice(["debug", "info", "warning", "critical"], case_sensitive=False)
)
