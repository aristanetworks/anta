...
async def collect_scheduled_show_tech(inv: AntaInventory, enable_pass: str, root_dir: str, tags: List[str],  # pylint: disable=too-many-arguments
                                      ssh_port: int, insecure: bool, configure: bool) -> None:
    """
    ...
    async def collect(device: InventoryDevice) -> None:
        ...
        try:
            ...
            commands = [{"cmd": "enable", "input": enable_pass}, f"bash timeout 30 zip {EOS_TECH_SUPPORT_ARCHIVE_ZIP} /mnt/flash/schedule/tech-support/*"]
            ...
            commands = [{"cmd": "enable", "input": enable_pass}, "show running-config | include aaa authorization exec default local"]
            res = await device.session.cli(commands=commands, ofmt='text')

            if not res[1]:
                if configure:
                    commands = [{"cmd": "enable", "input": enable_pass}, "configure terminal", "aaa authorization exec default local"]
                    await device.session.cli(commands=commands)
            ...
            commands = [{"cmd": "enable", "input": enable_pass}, f"bash timeout 30 rm {EOS_TECH_SUPPORT_ARCHIVE_ZIP}"]
            await device.session.cli(commands=commands)
