...
def from_cvp(ctx: click.Context, output: Path, cvp_ip: str, cvp_username: str, cvp_password: str, cvp_container: str, overwrite: bool) -> None:
    """
    Build ANTA inventory from Cloudvision

    TODO - handle get_inventory and get_devices_in_container failure
    """
    # It is CLI we don't care
    # pylint: disable=too-many-arguments
    _handle_overwrite(ctx, output, overwrite)
    ...
    create_inventory_from_cvp(cvp_inventory, output, cvp_container)
...
def _handle_overwrite(ctx: click.Context, output: Path, overwrite: bool) -> None:
    """
    When writing to a file, verify that it is empty, if not:
    * if a tty is present, prompt user for confirmation
    * else fail miserably

    If prompted and the answer is No, using click Abort mechanism
    """
