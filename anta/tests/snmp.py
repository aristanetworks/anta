...
async def verify_snmp_agent(device: InventoryDevice, result: TestResult, vrf: str = "default") -> TestResult:
    ...
    if not vrf:
        result.is_skipped(
            "verify_snmp_agent could not run because vrf was not supplied"
        )
        return result
...
async def verify_snmp_ipv4_acl(device: InventoryDevice, result: TestResult, number: int, vrf: str = "default") -> TestResult:
    ...
    if not number or not vrf:
        result.is_skipped(
            "verify_snmp_ipv4_acl could not run because number or vrf was not supplied"
        )
        return result
    ...
    if ipv4_acl_number != number:
        result.is_failure(
            f"Expected {number} SNMP ACL(s) for vrf {vrf} and got {ipv4_acl_number}"
        )
        return result
    ...
    if not_configured_acl_list:
        result.is_failure(
            f"SNMP ACL(s) not configured or active in vrf {vrf}: {not_configured_acl_list}"
        )
