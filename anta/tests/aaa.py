...
class VerifyAcctMethods(AntaTest):
    """Verifies the AAA accounting method lists for different accounting types (commands, exec, system, dot1x).

    This test verifies both the default and console accounting planes for each specified method list.

    Expected Results
    ----------------
    * Success: All specified accounting method lists match the expected default and/or console methods.
    * Failure: At least one method list is not found, not configured, or has methods that do not match.
    ...
    class Input(AntaTest.Input):
        """Input model for the VerifyAcctMethods test."""
    ...
    def test(self) -> None:
        ...
        for acct_entry in self.inputs.accounting:
            acct_type = acct_entry.acct_type
            # EOS key format: "commands" -> "commandsAcctMethods".
            methods = command_output.get(f"{acct_type}AcctMethods", {})
